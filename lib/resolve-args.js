...
module.exports = function(argv) {
  ...
  // versions
  if (argv.versioned) {
      var files = fs.readdirSync(argv.input);
      argv.versions = {};
      for (var v = 0 ; v < argv.versioned.length ; v++) {
          var versioned = argv.versioned[v];
          var versions = [];
          for (var f = 0 ; f < files.length ; f++) {
              var filename = files[f];
              if (filename.indexOf(versioned) === 0) {
                  var extInd = filename.indexOf(".md");
                  var version = filename.substr(versioned.length + 1, extInd - versioned.length - 1);
                  versions.push(version);
              }
          }
          if (versions.length) {
              argv.versions[versioned] = versions;
          }
      }
  }
  return argv;
