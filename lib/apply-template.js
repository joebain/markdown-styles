...
module.exports = function(opts) {
  ...
  return pi.forEach(function(item, enc, done) {
    ...
    // this is a hack, go tell your mum
    if (opts.versions) {
        var matches = /([^\-]*)-(.*)\.md/.exec(item.relative);
        if (matches && matches.length == 3) {
            var basename = matches[1];
            var version = matches[2];
            var versions = opts.versions[basename];
            var versionDetails = {};
            for (var v = 0; v < versions.length ; v++) {
                versionDetails[v] = {version: versions[v], selected: versions[v] == version};
            }
            item.versions = versionDetails;
        }
    }
